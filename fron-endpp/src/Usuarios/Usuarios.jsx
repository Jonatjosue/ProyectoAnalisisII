...
function Usuarios() {
  const { userRole, loggedInUser } = useContext(AuthContext);  // Get the logged-in user's info
  ...
  const [formData, setFormData] = useState({
    ...
    idSucursal: '',
    idStatusUsuario: '',
    fechaCreacion: '', // To be set automatically
    fechaModificacion: '',
    usuarioCreacion: '', // To be set automatically
    usuarioModificacion: '',
    roles: [], // Store selected roles here
    statusUsuario: '',
  });
  ...
  const [selectedRoleCount, setSelectedRoleCount] = useState(0); // Number of roles user wants to select
        ...
        <tbody>
          {usuarios.map((usuario) => (
            <tr key={usuario.idUsuario}>
              ...
              <td>
                {new Date(usuario.ultimaFechaCambioPassword).toLocaleDateString()}
              </td>
      ...
      <dialog ref={dialogRef}>
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <div className="form-group">
            ...
            <input type="number" className="form-control" name="sucursal" value={formData.sucursal} onChange={handleChange} />
          </div>
...
function Usuarios() {
  ...
  const openDialog = (usuario = null) => {
    if (usuario) {
      setFormData({ ...usuario, roles: usuario.roles || [] }); // Preload roles if editing
      setSelectedRoles(usuario.roles || []);
      setIsEditing(true);
    } else {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestOptions = {
        ...
        body: JSON.stringify({
          ...formData,
          roles: selectedRoles, // Include selected roles in the request
        }),
      };
      const url = isEditing
        ? `http://localhost:8081/api/Usuario/${formData.idUsuario}`
    ...
    <div className="container mt-5">
      ...
      <dialog ref={dialogRef}>
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <div className="form-group">
            <label>How many roles would you like to select?</label>
            <select
              className="form-control"
              value={selectedRoleCount}
              onChange={handleRoleCountChange}
            >
              ...
              {[...Array(availableRoles.length)].map((_, i) => (
